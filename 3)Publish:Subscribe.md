...
> 第一个参数就是交换机的名称。空字符串代表默认或者匿名交换机：消息将会根据指定的routingKey分发到指定的队列。
...
queueName可能是这样子的：amq.gen-U0srCoW8TsaXjNh73pnVAw==。
...
发布日志消息的程序看起来和之前的没有太大区别。最重要的改变就是我们把消息发送给logs交换机而不是匿名交换机。在发送的时候我们需要提供routing_key参数，但是它的值会被扇型交换机（fanout exchange）忽略。以下是EmitLog.java：
...
([EmitLog.java](https://github.com/rabbitmq/rabbitmq-tutorials/blob/master/java/EmitLog.java) 源文件)
...
([ReceiveLogs.java](https://github.com/rabbitmq/rabbitmq-tutorials/blob/master/java/ReceiveLogs.java) source)
